...
public interface FuncionRepo extends JpaRepository<Funcion, Integer> {
    ...
    @Query("select f.pelicula.nombre, f.pelicula.estado,f.sala.teatro.nombre, f.sala.teatro.ciudad.nombre,f.horario.dia  from Funcion f where f.pelicula.id =:codigoPelicula")
    List<Object[]> obtenerFuncionesPelicula(Integer codigoPelicula);
    @Query("select f from Funcion  f  where f.sala.teatro.id =:idTeatro and f.entradas is empty ")
    ...
    */
    @Query("select distinct f from Funcion f  where  f.pelicula.estado=:estado and f.sala.teatro.ciudad.nombre=:nombreCiudad")
